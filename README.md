...
| [`bridge-counterbore-hole/`](bridge-counterbore-hole/) | `slicing.slice_planner` + `slicing.perimeter_planner` | Stops a counterbore being walled in mid-air, either by closing the hole for one layer or by leaving the unsupported ring to the fill stage. **A reimplementation of OrcaSlicer's `counterbore_hole_bridging`, in both of its modes.** |
...
- `bridge-counterbore-hole` in its partial mode also takes `slicing.perimeter_planner`, so it
  displaces `alternate-extra-wall` as well as the three above.
- `cross-hatch`, `gradient-infill`, `radial-bridge`, `bridge-density` and `hull-line` are all
