-- Print 4 of 4: Prusa's own experiment.
-- Same settings as print 2, but slice the project with the deck split off by a modifier mesh:
-- prusa_benchy_hullline_test_block4_deck.3mf
-- The deck is then a region of its own, so the order becomes deck fill, hull wall, hull fill -
-- the wall back in the middle of the layer and the layer ending inside the part.
--
